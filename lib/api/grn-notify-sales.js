...
export async function notifyGrnPostedToSales(db, tenantId, grn) {
  ...
  if (!grn?.noDO) {
    return { skipped: true, reason: 'no_do' };
  }

  const payload = {
    customerTenantId: tid,
    vendorTenantId: grn.vendorTenantId || config.vendorTenantId,
    noDO: grn.noDO,
    noGRN: grn.noGRN,
    grnId: grn.id,
    noPO: grn.noPO || null,
    postedAt: grn.postedAt || new Date().toISOString(),
    receivedTotal: grn.receivedTotal || 0,
  ...
  const hutang = await upsertHutangFromSalesPayload(db, tid, data, grn, config);

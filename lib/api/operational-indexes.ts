...
const INDEX_SPECS: IndexSpec[] = [
  ...
  { collection: 'production_plans', index: { tenantId: 1, weeklyMenuPlanId: 1, tanggal: 1 }, name: 'idx_fp_plans_tenant_weekly_tanggal' },
  { collection: 'weekly_menu_plans', index: { tenantId: 1, id: 1 }, name: 'idx_weekly_menu_tenant_id' },
  { collection: 'weekly_menu_plans', index: { tenantId: 1, kitchenId: 1, weekStart: 1 }, name: 'uniq_weekly_menu_tenant_kitchen_week', unique: true },
  ...
  /** ADR-004 Fase 6 — forward trace lot → issue allocations. */
  {
    collection: 'material_issues',
  ...
  { collection: 'ingredient_lots', index: { tenantId: 1, supplierId: 1 }, name: 'idx_ilot_tenant_supplier' },
  ...
  { collection: 'people', index: { tenantId: 1, id: 1 }, name: 'idx_people_tenant_id' },
  { collection: 'people', index: { tenantId: 1, kode: 1 }, name: 'uniq_people_tenant_kode', unique: true, partialFilterExpression: { kode: { $type: 'string', $gt: '' } } },
  { collection: 'people', index: { tenantId: 1, nik: 1 }, name: 'uniq_people_tenant_nik', unique: true, partialFilterExpression: { nik: { $type: 'string', $gt: '' } } },
  {
    collection: 'people',
    index: { tenantId: 1, 'bankAccounts.bankCode': 1, 'bankAccounts.accountNo': 1 },
    name: 'uniq_people_tenant_bank_acc',
    unique: true,
    partialFilterExpression: { 'bankAccounts.accountNo': { $type: 'string', $gt: '' } },
  },
  { collection: 'people', index: { tenantId: 1, aktif: 1, nama: 1 }, name: 'idx_people_tenant_aktif_nama' },
  { collection: 'people', index: { tenantId: 1, kitchenIds: 1 }, name: 'idx_people_tenant_kitchen' },
  {
    collection: 'person_payments',
    index: { tenantId: 1, bankRef: 1 },
    name: 'uniq_person_pay_tenant_bankref',
    unique: true,
    partialFilterExpression: { bankRef: { $type: 'string', $gt: '' } },
  },
  { collection: 'person_payments', index: { tenantId: 1, personId: 1, tanggal: -1 }, name: 'idx_person_pay_tenant_person_tanggal' },
  { collection: 'bank_txn_inbox', index: { tenantId: 1, id: 1 }, name: 'idx_bank_txn_tenant_id' },
  {
    collection: 'bank_txn_inbox',
    index: { tenantId: 1, provider: 1, bankRef: 1 },
    name: 'uniq_bank_txn_tenant_provider_ref',
    unique: true,
    partialFilterExpression: { bankRef: { $type: 'string', $gt: '' } },
  },
  { collection: 'bank_txn_inbox', index: { tenantId: 1, status: 1, valueDate: -1 }, name: 'idx_bank_txn_tenant_status_date' },
  ...
  { collection: 'distribution_orders', index: { tenantId: 1, productionResultId: 1, createdAt: -1 }, name: 'idx_dist_tenant_result' },
  {
    collection: 'distribution_orders',
    index: { tenantId: 1, 'fefoConsume.allocations.batchId': 1 },
    name: 'idx_dist_tenant_batch_alloc',
  },
  ...
  { collection: 'haccp_plans', index: { tenantId: 1, noDokumen: 1 }, name: 'uniq_haccp_plan_tenant_no', unique: true },
  /** ADR-004: max satu plan ACTIVE per tenant (versioning inline + supersede). */
  {
    collection: 'haccp_plans',
    index: { tenantId: 1 },
    name: 'uniq_haccp_plan_tenant_active',
    unique: true,
    partialFilterExpression: { status: 'ACTIVE' },
  },
  { collection: 'haccp_verifications', index: { tenantId: 1, id: 1 }, name: 'idx_haccp_ver_tenant_id' },
  { collection: 'haccp_verifications', index: { tenantId: 1, noDokumen: 1 }, name: 'uniq_haccp_ver_tenant_no', unique: true },
  { collection: 'haccp_verifications', index: { tenantId: 1, verificationType: 1, verifiedAt: -1 }, name: 'idx_haccp_ver_type' },
  { collection: 'haccp_verifications', index: { tenantId: 1, haccpPlanId: 1, verifiedAt: -1 }, name: 'idx_haccp_ver_plan' },
  { collection: 'haccp_verifications', index: { tenantId: 1, haccpResultId: 1, verifiedAt: -1 }, name: 'idx_haccp_ver_result' },
  ...
  { collection: 'qc_results', index: { tenantId: 1, checklistPeriod: 1, category: 1 }, name: 'idx_qc_res_period' },
  { collection: 'qc_results', index: { tenantId: 1, requirementId: 1, checklistPeriod: 1 }, name: 'idx_qc_res_requirement_period' },

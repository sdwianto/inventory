...
export async function resolveVendorProductMap(db, tenantId, vendorTenantId, vendorItem) {
  ...
  if (vendorItem.kode) {
    const prodFilter = { tenantId: tid, kode: vendorItem.kode, aktif: { $ne: false } };
    if (vendorTenantId) prodFilter.vendorTenantId = vendorTenantId;
    const prod = await db.collection('products').findOne(prodFilter);
    if (!prod && vendorTenantId) {
      ...
      if (fallback) {
        return { localStokId: fallback.id, localKode: fallback.kode, localNama: fallback.nama };
      }
    }
    if (prod) {

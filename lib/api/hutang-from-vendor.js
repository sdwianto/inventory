...
async function loadPoVarianceContext(db, tid, payload, vendorTenantId = null) {
  ...
  const salesOrderTotal = parseInt(payload.salesOrderTotal || 0, 10);
  if (salesOrderTotal > soTotal) {
    soTotal = salesOrderTotal;
    if (!soSubTotal) soSubTotal = parseInt(payload.salesOrderSubTotal || 0, 10) || salesOrderTotal;
  }
  ...
  await db.collection('hutang').updateOne(
    { id: existing.id },
    {
      $set: {
        ...
        salesOrderTotal: parseInt(payload.salesOrderTotal || 0, 10) || existing.salesOrderTotal || null,
        salesOrderSubTotal: parseInt(payload.salesOrderSubTotal || 0, 10) || existing.salesOrderSubTotal || null,
...
export async function createHutangFromVendorInvoice(db, customerTenantId, payload, vendorTenantId) {
  ...
  const hutang = stampTenantId(tid, {
    ...
    salesOrderId: payload.salesOrderId || null,
    salesOrderTotal: parseInt(payload.salesOrderTotal || 0, 10) || null,
    salesOrderSubTotal: parseInt(payload.salesOrderSubTotal || 0, 10) || null,
  ...
  const ppnAmt = parseInt(hutang.ppn || 0, 10);
  const totalAmt = parseInt(hutang.total || 0, 10);
  ...
  try {
    ...
    // Jurnal opsional di inventory customer — jangan batalkan hutang vendor.
    console.warn('[hutang-from-vendor] jurnal dilewati:', e?.message || e);
  }

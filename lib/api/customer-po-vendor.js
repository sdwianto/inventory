// Siapkan baris PO untuk dikirim ke sales.app — vendorTenantId per produk dari katalog global.
...
export async function enrichPoItemsForVendor(db, tenantId, items) {
  ...
  for (const it of items || []) {
    ...
    if (!prod && (it.kode || it.vendorKode)) {
      ...
      prod = await db.collection('products').findOne({ tenantId: tid, kode, syncSource: 'sales.app' });
    }
    ...
    if (!vendorStokId || !itemVendorTenantId) {
      const map = await db.collection('vendor_product_map').findOne({
        tenantId: tid,
        ...(prod?.id ? { localStokId: prod.id } : {}),
        ...(vendorKode ? { vendorKode } : {}),
      });
      if (map) {
        enriched.push({
          kode: map.vendorKode || vendorKode,
          vendorStokId: map.vendorStokId,
          vendorTenantId: map.vendorTenantId,
      ...
      return {
        error: `Produk "${it.nama || vendorKode || it.localStokId}" belum terhubung ke katalog sales.app. Jalankan Sync Katalog di menu Integrasi.`,
      };
    }

...
import { requireAuth, requireRole, STOCK_ADJUST_ROLES } from '@/lib/api/require-auth';
...
import { warehouseLabel, WAREHOUSE_CODES, normalizeWarehouseKode, ensureWarehousesForTenant } from '@/lib/api/warehouses';
...
export async function handleInventory({ db, route, method, path, body, url, auth, request }) {
  // ---------- SALDO PER GUDANG ----------
  ...
  if (route === '/stok/produksi' && method === 'POST') {
    ...
    for (const b of bahan) {
      const prod = await findMasterDoc(db, 'products', scopeAuth, { id: b.stokId });
    ...
    for (const h of hasil) {
      const prod = await findMasterDoc(db, 'products', scopeAuth, { id: h.stokId });
  ...
  if (path[0] === 'lokasi' && path.length === 2) {
    ...
    if (method === 'PUT') {
      ...
      const kode = normalizeWarehouseKode(lokExisting.kode);
    ...
    if (method === 'DELETE') {
      if (access.error) return access.error;
      const kode = normalizeWarehouseKode(access.doc.kode);
      if (WAREHOUSE_CODES.includes(kode)) {
  ...
  if (route === '/lokasi/bulk-delete' && method === 'POST') {
    const { denied, scopeAuth } = resolveOperationalScope(auth, { url, body, request });
    if (denied) return denied;
    return bulkDeleteMaster(db, scopeAuth, 'lokasi', body?.ids);
  }
  ...
  if (route === '/stok/transfer' && method === 'POST') {
    ...
    for (const it of items) {
      const prod = await findMasterDoc(db, 'products', scopeAuth, { id: it.stokId });
      if (!prod) return err(`Produk ${it.kode || it.stokId} tidak ditemukan`, 404);
      const whCheckAsal = assertProductWarehouse(prod, body.lokasiAsal);
      if (whCheckAsal) return err(whCheckAsal.error, 400);
      const whCheckTujuan = assertProductWarehouse(prod, body.lokasiTujuan);

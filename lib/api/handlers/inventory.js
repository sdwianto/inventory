...
export async function handleInventory({ db, route, method, path, body, url, auth }) {
  ...
  if (route === '/stok/kartu' && method === 'GET') {
    ...
    if (productId) {
      ...
      if (p) {
        product = clean(p);
        const tid = p.tenantId || auth?.tenantId || 'default';
  ...
  if (route === '/stok/kartu/reconcile' && method === 'POST') {
    const denied = requireRole(auth, STOCK_ADJUST_ROLES);
    if (denied) return denied;
    ...
    const product = clean(await findMasterDoc(db, 'products', auth, { id: productId }));

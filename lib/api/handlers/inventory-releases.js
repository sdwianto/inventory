...
import { tenantIdForWrite, withTenantFilter, findMasterDoc } from '@/lib/api/tenant-master';
...
async function loadRelease(db, auth, id) {
  return db.collection('inventory_releases').findOne(withTenantFilter(auth, { id }));
}

export async function handleInventoryReleases({ db, route, method, path, body, auth }) {
  ...
  if (path[0] === 'inventory-releases' && path.length === 2 && method === 'GET') {
    const denied = requireAuth(auth);
    if (denied) return denied;
    const doc = await loadRelease(db, auth, path[1]);
  ...
  if (route === '/inventory-releases' && method === 'POST') {
    const denied = requireRole(auth, RELEASE_CREATE_ROLES);
    if (denied) return denied;

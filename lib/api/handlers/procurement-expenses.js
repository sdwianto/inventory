...
export async function handleProcurementExpenses({ db, route, method, url, auth }) {
  if (route !== '/procurement-expenses' || method !== 'GET') return null;

  const denied = requireRole(auth, REPORT_ROLES);
  if (denied) return denied;

  const tenantId = auth.isMaster ? null : (auth.tenantId || 'default');
  ...
  let filter = withTenantFilter(auth, { referenceType: 'VENDOR_INVOICE' });
  ...
  const poTenant = tenantId || list[0]?.tenantId || 'default';

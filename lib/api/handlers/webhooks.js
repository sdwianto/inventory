...
export async function handleWebhooks({ db, route, method, request, body }) {
  ...
  try {
    ...
    if (CPO_EVENTS.has(event)) {
      const cpoPayload = {
        ...payload,
        // vendorTenantId dari body (tenant pengirim webhook / sales.app).
        vendorTenantId: body?.tenantId,
      };
      const cpoSync = await syncCpoFromVendorEvent(db, customerTenantId, event, cpoPayload);
    ...
    if (PRODUCT_EVENTS.has(event) && payload.product) {
      ...
      result = {
        ...
        fallback: true,
        message: fallbackMsg,
        cpoStatus: result.cpoSync?.status,
      };

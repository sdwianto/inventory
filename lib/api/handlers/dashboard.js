...
export async function handleDashboard({ db, route, method, auth }) {
  ...
  const tenantPo = withTenantFilter(auth, {});
  const tenantGrn = withTenantFilter(auth, {});
  const tenantProducts = withTenantFilter(auth, { aktif: true });
  const tenantStok = withTenantFilter(auth, { lokasiKode: { $in: WAREHOUSE_CODES } });
  const tenantHutang = withTenantFilter(auth, {
  ...
  const [
    ...
    db.collection('hutang').countDocuments(
      withTenantFilter(auth, { referenceType: 'VENDOR_INVOICE', approvalStatus: 'PENDING_REVIEW' }),
    ),
    db.collection('hutang').aggregate([
      {
        $match: {
          ...withTenantFilter(auth, { referenceType: 'VENDOR_INVOICE' }),
          $or: [

...
export async function handleGoodsReceipts({ db, route, method, path, body, url, auth }) {
  ...
  if (path[0] === 'goods-receipts' && path[2] === 'post' && method === 'POST') {
    ...
    for (const it of (grn.items || [])) {
      ...
      const bodyLine = body?.items?.find((b) => b.lineId === it.lineId);
    ...
    await db.collection('goods_receipts').updateOne(
      ...
      { $set: { status: 'POSTED', items: itemsFull, lokasi: lokasiSummary, lokasiKodes: [...lokasiSet], postedAt: now, userName: body?.userName } },
    ...
    return ok(clean({ ...posted, cpoSync }));

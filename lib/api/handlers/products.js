...
import { getStokByWarehouseBatch, syncProductStokFromLokasi } from '@/lib/api/stok-lokasi';
  ...
  if (path[0] === 'products' && path.length === 2) {
    ...
    if (method === 'PUT') {
      ...
      if (stokDiubah) {
        ...
        await setProductWarehouseStock(db, tid, id, gudang, parseFloat(update.stok || 0));

...
function parseDateParam(val: string | null, endOfDay = false): Date | null {
  if (!val) return null;
...
export async function handleProcurementExpenses({
  db, route, method, url, auth, request,
}: HandlerContext) {
  ...
  const inRange = (h: HutangDoc) => {
    ...
    const dt = h.approvedAt
      ? new Date(h.approvedAt)
      : new Date(String(h.tanggal || h.createdAt || ''));

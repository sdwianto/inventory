...
/** Produk sync sales.app: unik per vendorTenantId + vendorStokId.
 *  Catatan: keunikan kode produk lokal divalidasi di level aplikasi (handler products),
 *  karena MongoDB partial index tidak mendukung "syncSource tidak ada" ($exists:false / $not). */
export async function ensureProductCatalogIndexes(db) {
  await dropIndexIfExists(db, 'products', 'uniq_products_tenant');
  // Index lama ini memakai $exists:false yang ditolak MongoDB — pastikan dibuang.
...
/** MASTER: tenantId dari query → body → cookie acting; wajib untuk operasional. */
export function resolveMasterActingTenantId(auth, { url, body, request } = {}) {
  if (!auth?.isMaster) return null;
  const fromQuery = url?.searchParams?.get('tenantId')?.trim() || '';
  const fromBody = body?.tenantId ? String(body.tenantId).trim() : '';
  const fromCookie = readActingTenantFromRequest(request);
 ...
 * @returns {{ denied: import('next/server').NextResponse | null, scopeAuth: object | null, tenantId: string | null }}
 */
export function resolveOperationalScope(auth, { url, body, request } = {}) {
  const denied = requireAuth(auth);
  if (denied) return { denied, scopeAuth: null, tenantId: null };

  if (auth.isMaster) {
    const acting = resolveMasterActingTenantId(auth, { url, body, request });
    if (!acting) {
      return {
    ...
    return {
      denied: null,
      scopeAuth: authForMasterActing(auth, acting),
      tenantId: acting,
    };
  }

  const tenantId = normalizeTenantId(auth.tenantId || 'default');
  return { denied: null, scopeAuth: auth, tenantId };
}

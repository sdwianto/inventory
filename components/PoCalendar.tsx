...
import {
  dateKey,
  groupPosByArrivalDate,
  PO_STATUS_ORDER,
  statusesOnDay,
  type PoArrivalFields,
} from '@/lib/po-calendar';
import PoStatusDot, { PoStatusLegendItem } from '@/components/PoStatusDot';

type PoCalendarContextValue = {
  poByDate: Record<string, PoArrivalFields[]>;
  selectedKey: string | null;
  onCreateForDate: (date: Date) => void;
};

const PoCalendarContext = createContext<PoCalendarContextValue | null>(null);

function usePoCalendarContext() {
  const ctx = useContext(PoCalendarContext);
  if (!ctx) throw new Error('PoDayButton must be used within PoCalendar');
  return ctx;
}

/** Stable component ref — jangan buat inline di `components` (menyebabkan remount/blink saat hover). */
function PoDayButton({ day, modifiers, className, ...props }: DayButtonProps) {
  const { poByDate, selectedKey, onCreateForDate } = usePoCalendarContext();
...
export default function PoCalendar({
  ...
  const ctx = useMemo(
    () => ({ poByDate, selectedKey, onCreateForDate }),
    [poByDate, selectedKey, onCreateForDate],
  );

  return (
    <PoCalendarContext.Provider value={ctx}>

...
function SummaryCard({ title, qty, nilai, qtyClass = 'text-slate-800', nilaiClass = 'text-slate-600' }) {
  return (
...
export default function SaldoGudangPage() {
  ...
  const filteredTotals = useMemo(() => {
    ...
    for (const r of stockRows) {
      qty += parseFloat(r.stokQty ?? r.stokTotal ?? 0) || 0;
      nilai += parseFloat(r.nilaiStok ?? r.nilaiTotal ?? 0) || 0;
    }

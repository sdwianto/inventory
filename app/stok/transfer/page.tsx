      ...
      <div className="p-4 md:p-6 space-y-4">
        ...
        <ListSummaryCards
          items={[
            ...
            { label: 'Total Item Dipindah', value: list.reduce((s, d) => s + asArray(d.items).length, 0), colSpan: 2 },
          ]}
        />
            ...
            <tbody>
              ...
              {list.map(d => (
                <tr key={str(d.id)} className="border-t hover:bg-slate-50">
                  ...
                  <td className="px-3 py-2 font-mono text-xs">{str(d.noTransfer)}</td>
                  <td className="px-3 py-2">{str(d.lokasiAsalNama)}</td>
        ...
        <DialogContent className="max-w-3xl max-h-[90vh] flex flex-col">
          ...
          <div className="space-y-3 overflow-y-auto">
            <div className="grid grid-cols-[1fr_auto_1fr] gap-3 items-end">
              <div><Label>Dari Lokasi *</Label><select value={form.lokasiAsal} onChange={e => setForm({...form, lokasiAsal: e.target.value})} className="w-full border rounded px-3 py-2 text-sm"><option value="">-- pilih --</option>{lokasi.map(l => <option key={str(l.id)} value={str(l.kode)}>{str(l.kode)} - {str(l.nama)}</option>)}</select></div>
              <ArrowRight className="w-5 h-5 text-orange-500 mb-2" />
              <div><Label>Ke Lokasi *</Label><select value={form.lokasiTujuan} onChange={e => setForm({...form, lokasiTujuan: e.target.value})} className="w-full border rounded px-3 py-2 text-sm"><option value="">-- pilih --</option>{lokasi.map(l => <option key={str(l.id)} value={str(l.kode)}>{str(l.kode)} - {str(l.nama)}</option>)}</select></div>
            </div>

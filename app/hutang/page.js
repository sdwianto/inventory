...
export default function HutangVendorPage() {
  ...
  const doApprove = async () => {
    ...
    setActing('approve');
    try {
      const res = await fetch(`/api/hutang/${detail.id}/approve`, {
        method: 'POST',
  ...
  const doReject = async () => {
    ...
    setActing('reject');
    try {
      const res = await fetch(`/api/hutang/${detail.id}/reject`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  const markPaidById = async (id, noInvoice) => {
  ...
  const doMarkPaid = async () => {
    if (!detail) return;
    await markPaidById(detail.id, detail.noInvoice);
  };
      ...
      <Dialog open={!!detail} onOpenChange={(o) => !o && setDetail(null)}>
        <DialogContent className="max-w-4xl max-h-[94vh] overflow-y-auto p-0 gap-0">
          <DialogHeader className="sr-only">
            <DialogTitle>
              {detail ? `Faktur tagihan ${detail.noInvoice}` : 'Detail tagihan vendor'}
            </DialogTitle>
          </DialogHeader>
          {detail && (
            <VendorInvoiceDetail

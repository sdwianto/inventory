...
describe('EE-14 Phase 2 — Inventory registry consumer gate', () => {
  ...
  it('ee12-install pins events + platform versions for registry path', () => {
    ...
    expect(script).toContain('@sdwianto/metrics@${METRICS_VERSION}');
  ...
  it('platform vendor package is 1.0.4 with events + metrics dependency', () => {
    ...
    expect(pkg.dependencies['@sdwianto/events']).toBe('1.0.1');
    expect(pkg.dependencies['@sdwianto/metrics']).toBe('1.0.0');
  });

  it('ee13 registry gate documents platform 1.0.4 consumer path', () => {
    ...
    expect(gate).toContain("const PLATFORM_VERSION = '1.0.4'");
  });
});

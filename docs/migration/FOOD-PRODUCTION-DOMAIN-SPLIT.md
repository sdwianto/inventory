...
- [x] **`rencana-kebutuhan.ts` vs `material-requirement.ts`** — diinvestigasi, **tidak dikonsolidasi** (koreksi dari rencana awal). Bukan duplikasi murni: `rencana-kebutuhan.ts` mengagregasi **banyak Plan sekaligus** untuk preview/print (`buildRencanaKebutuhanLines(plans: [...])`), sedangkan `material-requirement.ts`'s `explodeMaterialRequirements()` bekerja per **satu Plan** untuk dokumen MRP resmi. Keduanya sudah pure function dan sudah berbagi helper level-bawah (`scaleRecipeIngredientQty`, `roundQty`). Konsolidasi yang aman berarti menulis ulang loop multi-plan agar memanggil `explodeMaterialRequirements()` per plan lalu digabung — refactor nyata, bukan sekadar hapus duplikasi, berisiko mengubah angka kebutuhan bahan. **Ditunda**, tetap tercatat sebagai technical debt "kerjakan kapan longgar" sesuai rencana awal — sengaja tidak dipaksakan di sesi ini.
  - **Update 2026-07-28 (post-closeout)**: dikerjakan, dengan pendekatan lebih konservatif dari rencana awal (bukan "rencana-kebutuhan panggil explodeMaterialRequirements per plan" — itu tetap berisiko karena beda kontrol alur: fail-fast vs permissive, cek `recipe.aktif` vs tidak, `targetPorsi<=0` = error vs skip diam-diam, rounding per-step vs di akhir). Yang diekstrak: **hanya** rumus skala resep per bahan (besar+kecil, override/exclude/buffer) yang secara literal identik di kedua file, jadi `computeRecipeLineContributions()` (baru, `material-requirement.ts`) — pure, dipanggil kedua fungsi, kontrol alur masing-masing (fail-fast/permissive, aktif-check, rounding) tidak disentuh. Duplikasi kode hilang, angka tidak berubah — diverifikasi lewat `tests/unit/food-production-sprint4.test.ts` (assertion numerik `qtyGross`/`qtyNet`/override/exclude/buffer/inactive-recipe tetap sama persis) + `tsc --noEmit` clean.
